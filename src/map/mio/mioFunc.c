...
***********************************************************************/
int Cad_Mio_GateParseFormula( Mio_Gate_t * pGate )
{
    char * pPinNames[100];
    char * pPinNamesCopy[100];
    Mio_Pin_t * pPin, ** ppPin;
    ...
    pGate->dDelayMax = 0.0;
    nPins = 0;
    // cad note : might be one of the attribute
    Mio_GateForEachPin( pGate, pPin )
    {
        // set the maximum delay of the gate
        if ( pGate->dDelayMax < pPin->dDelayBlockMax )
...
***********************************************************************/
int Cad_Mio_LibraryParseFormulas( Mio_Library_t * pLib )
{
    Mio_Gate_t * pGate;
    ...
    // for each gate, derive its function
    Mio_LibraryForEachGate( pLib, pGate )
        if ( Cad_Mio_GateParseFormula( pGate ) )
            return 1;
    return 0;
}
